...
if FileExist(COMMON_PATH.."MixLib.lua") then
 ...
 DownloadFileAsync("https://raw.githubusercontent.com/VTNEETS/GoS/master/MixLib.lua", COMMON_PATH.."MixLib.lua", function() PrintChat("Downloaded MixLib. Please 2x F6!") return end)
end 
...
local CassioW = {delay = 0.5, range = 800, radius = 160, speed = 2500}
...
function Combo()
	if Mix:Mode() == "Combo" then
		...
		if CM.C.W:Value() and ValidTarget(target, 800) and Ready(_W) and qTime < (GetGameTimer() - 1) and not target.isPoisoned then
			CastW(target)
		end
